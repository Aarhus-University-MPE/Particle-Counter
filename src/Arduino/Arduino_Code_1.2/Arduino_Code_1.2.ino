...
This program is intended for Arduino UNO R3 or Leonardo R3
(or Olimexino, which is a Leonardo clone). Edit #defines to set which one. If
using Arduino UNO R3 or Leonardo R3, the different communication logic levels should
be respected. Arduino is 5v, OPC-N3 is 3.3v. The OPC-N3 PIC is 5v tolerant on most
of the SPI pins, but not the Slave Select pin. A simple solution is to use a
resistor pair to divide the voltage on the SS pin appropriately. If an Olimexino device
is used, it has a jumper switch to set it to either 5v or 3.3v. Set to 3.3v to
connect to OPC-N3
without the need for logic level conversion.
The program is for use with OPC-N3 devices with standard
v1.17a firmware.
This program configures the Arduino to put an OPC-N3 through
1-minute cycles
taking one set of measurements per second during the 10s
active part of the cycle
and turning the fan and laser off for the remainder of the
cycle.
The data taken from the OPC-N3 via the SPI interface is
processed
and the processed data fed to the serial interface (the USB
port) so that it can be displayed on a computer using
suitable terminal software.
Usual serial port settings should be set in the terminal
software (8 data bits,
1 stop bit, no parity, no flow control). Set the baud rate to
match the value
defined in this program.
Adding more Slave Select pins will allow multiple OPC-N3
devices on one SPI bus.
...
#define SPI_OPC_busy  0x31  // Hexadecimal for decimal= 49, eller binær= 0011 0001
#define SPI_OPC_ready 0xF3  // Hexadecimal for decimal= 243, eller binær= 1111 0011
...
void setup() {
  ...
  */
  ...
  // Start serial port
  opSerial.begin(BaudRate);  // Same as serial.begin(9600), just with variables in case of other system configurations read the first text.
  ...
  */
  ...
  ssPin_OPC = 10;
  wdt_reset();   // Reset watchdog timer
  InitDevice();  // Starts that function
  wdt_reset();   // Reset watchdog timer
  // END Device #1
...
void InitDevice() {
  wdt_reset();              // Reset watchdog timer
  ReadOPCstring(0x10);      // Get serialstr from OPC device, communication "variable" decimal = 160.
  ReadOPCstring(0x3F);      // Get infostr from OPC device, communication "variable" decimal = 63.
...
void loop() {
  ...
  if (currentTime >= cloopTime) {
    ...
    wdt_reset();         // Reset watchdog timer
    // Device #1 (ssPin_OPC = 10) Kunne pinnen ikke bare låses til pin 10 hvorfor bliver den læst flere gange?
    ssPin_OPC = 10;
    ...
    for (byte i = 0; i < 10; i++) {
      ...
      if (i != 0) {
        // Print time since start (millis() returns an unsigned long of the number of ms since the program started. It wraps around in ~50 days.)
...
void StartOPC() {
  ...
  SPI.endTransaction();
  delay(10);
  delay(1000);  // Delay may be necessary to separate power ON of fan and laser
  // Fan DAC ON
  GetReadyResponse(0x03);
