...
// #define Serial Serial1 //(On Leonardo or Olimexino, 'Serial' is distinct from 'Serial1'. 'Serial' uses the USB link and 'Serial1' uses the serial io pins D0 and D1. On Uno, 'Serial' uses both simultaneously.)
...
SoftwareSPI OPC_spi(OPC_SCK, OPC_MOSI, OPC_MISO, OPC_n3_SS);
...
void InitSD();
void OpenSD(File &FileName);
void CloseSD(File &FileName);
...
void setup() {
  ...
  PrintFirmwareVer(Serial, SysData);  // Prints to serial and SysData file
  ...
  //Close the SysData file before generating the Storage file.
  CloseSD(SysData);
  // At this point the system should be initialiced and alle SysData should be in the file.
  // The Storage file can then be initiated.
  OpenSD(Storage);

  // PrintDataLabels(Serial);  // Print labels to serial port and storage file

  CloseSD(Storage);

  currentTime = millis();
  cloopTime   = currentTime;
  Serial.println("Done with setup loop");
}
...
void OpenSD(File &FileName) {
  if (FileName == SysData) {
    FileName = SD.open("SysData.txt", FILE_WRITE | O_CREAT | O_APPEND);  // Edit file name to contain date.
    if (!FileName) {
    ...
    FileName = SD.open("Storage.csv", FILE_WRITE | O_CREAT | O_APPEND);  // Edit file name to contain date.
    if (!FileName) {
...
void PrintOPCstring(Stream &port) {
  // OpenSD(SysData, SD_SS);
  SysData.write(SPI_in, 60);
