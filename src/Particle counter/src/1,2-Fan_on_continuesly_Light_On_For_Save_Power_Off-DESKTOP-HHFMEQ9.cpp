...
/*
This Program have been changed a considerable amount by Jeppe Fogh Rasmussen at Aarhus University.
In addition to connecting thru a arduino to the OPC, there have been incoorporated both a RTC and a SD memory module.
To accomidate the SD Module the original code have been changed from the dedicated SPI bus,
and the OPC connection is now beeing done by the method of bitbanging.
This is due to the sd card not working when multible units are on the SPI bus.
